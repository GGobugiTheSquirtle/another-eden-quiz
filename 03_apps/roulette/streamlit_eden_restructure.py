...
def slot_machine_display(items, winner_index, item_display_duration_ms=50, spin_duration_s=3):
    ...
    (function() {{
        ...
        // 레거시 기능: 스핀 시작 시 사운드 재생 (오류 발생 시 무시)
...
def get_image_base64(image_path):
    ...
    try:
        ...
        log_debug(f"Base64 encoding error for {image_path}: {e}")
        return None
...
def main():
    ...
    # 정확한 데이터 파일 경로 사용
    # 통합 데이터 파일 우선 시도, 없으면 기존 파일들 시도
    default_csv_path = CSV_DIR / "eden_unified_data.csv"
    if not default_csv_path.exists():
        default_csv_path = CSV_DIR / "eden_roulette_data.csv"
    if not default_csv_path.exists():
        default_csv_path = CSV_DIR / "eden_quiz_data.csv"
